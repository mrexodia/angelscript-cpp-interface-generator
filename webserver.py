...
SVN_REVISION = "$Rev: 13 $"
SVN_ID = "$Id: webserver.py 13 2009-08-02 00:58:10Z RoRTom $"
...
<tr><td>Namespace Mode</td><td>
	<select name="namespacemode">
	 <option value="0" selected>Add Namespaces to class names</option>
	 <option value="1">Just the pure class names, no namespaces</option>
	</select></td>
</tr>
...
<div style="font-size:x-small;color:#aaaaaa;position:absolute;right:0px;bottom:0px;">(ASIG revision %(rev)s)</div>
    ...
    def do_POST(self):
        try:
            ...
            self.wfile.write(
                "<html><body>got the file, processing (this can take several seconds)...<p>")
            self.wfile.flush()
            ...
            if ext.lower() in '.h .hh .hpp .hxx .cpp .cxx .cc .c .txt'.split():
                ...
                if not zipfile.is_zipfile(dstfile):
                    self.wfile.write(
                        "error: uploaded file is no valid zip file!<p>")
                    return
                ...
                self.wfile.write(
                    "only .zip Archives or source/header files allowed. Please go back and try again.")
                self.wfile.flush()
                return
            ...
            self.wfile.write(
                "<br/>worker started, this can take some time<br/>")
            self.wfile.flush()
